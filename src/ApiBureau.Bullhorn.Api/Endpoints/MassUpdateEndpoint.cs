...
public sealed class MassUpdateEndpoint
{
    ...
    public Task<HttpResponseMessage> GetAsync(CancellationToken token) => _client.GetAsync($"/{RequestUrl}", token);
    ...
    public Task<HttpResponseMessage> GetAsync(EntityType type, CancellationToken token)
        => _client.GetAsync($"/{RequestUrl}/{type}", token);
    ...
    public Task<HttpResponseMessage> UpdateAsync(EntityType type, object data, CancellationToken token)
        => _client.PostAsJsonAsync($"/{RequestUrl}/{type}", data, token);
}